...
# -- Linhas de execução para instalação das ferramentas necessárias para o Hardening
apt-get update -y
apt-get install -y iptables ssh curl wget xinetd

# -- Hardening do IPTABLES
...
# Cria arquivo com as regras atualizadas do IPTABLES
mkdir -p /etc/iptables
touch /etc/iptables/rules.v4
iptables-save > /etc/iptables/rules.v4

# -- Hardening do Docker: Conatiner Web Apache
